...
CASE
    WHEN regexp_like(etf.sector_productivo,'Gastronomía, Hotelería y Turismo|Camareros') or regexp_like(etf.descripcion,'Camarero/a|Cocinero/a|Bachero/a') THEN 'GASTRONOMIA, HOTELERIA Y TURISMO'
    ...
    WHEN regexp_like(etf.sector_productivo,'Contabilidad|Recursos Humanos y Capacitación|Secretarias y Recepción|Administración, Contabilidad y Finanzas|Gerencia y Dirección General') THEN 'ADMINISTRACION, CONTABILIDAD Y FINANZAS'
    WHEN regexp_like(etf.sector_productivo,'Aduana y Comercio Exterior|Marketing y Publicidad|Atención al Cliente, Call Center y Telemarketing|Ventas|Comercial, Ventas y Negocios|Comercial') THEN 'COMERCIAL, VENTAS Y NEGOCIOS'
    ...
    WHEN regexp_like(etf.sector_productivo,'Hipódromo de Palermo') THEN 'HIPODROMO'
    ...
    WHEN regexp_like(etf.sector_productivo,'Minería, Energía, Petróleo y Gas') THEN 'MINERIA, ENERGIA, PETROLEO Y GAS'
    WHEN regexp_like(etf.sector_productivo,'Oficios y Otros') THEN 'OFICIOS Y OTROS'
    WHEN regexp_like(etf.sector_productivo,'Ingenierías|Producción y Manufactura') THEN 'PRODUCCION Y MANUFACTURA'
