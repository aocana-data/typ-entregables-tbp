...
SELECT p.id AS id_programa,
       p.ministerio_id AS id_ministerio,
       p.reparticion_id AS id_reparticion,
...
ON (cd.id = c.id)
LEFT JOIN "caba-piba-staging-zone-db"."tbp_typ_def_programa" p ON (p.id_programa = c.programa_id)
...
SELECT 'CRMSL' AS base_origen,
	...
	CAST(OF.id AS VARCHAR) AS id_capacitacion,
...
SELECT 'SIENFO' AS base_origen,
	...
	-- CAST(ca.id_carrera AS VARCHAR) AS id_capacitacion,
	CAST(ca.id_carrera AS VARCHAR) || '-' || CAST(ca.id_carrera AS VARCHAR) AS id_capacitacion,
...
SELECT 'SIENFO' AS base_origen,
	'CURSO' AS tipo_capacitacion,
	CAST(COALESCE(t.id_carrera, 0) AS VARCHAR) || '-' || CAST(cu.id_curso AS VARCHAR) AS id_capacitacion,
...
SELECT 'MOODLE' AS base_origen,
	...
	'CURSO' AS tipo_capacitacion,
	CAST(cc.id AS VARCHAR) AS id_capacitacion,
...
CREATE TABLE "caba-piba-staging-zone-db"."tbp_typ_tmp_siu_1" AS
	WITH siu AS (
			SELECT 'SIU' AS base_origen,
				'CARRERA' AS tipo_capacitacion,
				CAST(spl.PLAN AS VARCHAR) AS id_capacitacion,
...
SELECT tc.id id_capacitacion,
	...
	tc.fecha_fin,
	tc.estado,
	ca.id_capacitacion AS id_capacitacion_asi,
...
SELECT 'GOET' base_origen,
	    ...
	    CONCAT(RPAD(CASE WHEN (goettd.detalle IN ('DNI', 'CUIT', 'CUIL', 'LC', 'LE', 'CI')) THEN goettd.detalle
				...
				LPAD((CASE WHEN goettd.detalle IN ('DNI',
                                         ...
                                         'CI',
                                         'CUIT',
                                         'CUIL') THEN REGEXP_REPLACE(UPPER(goetu.ndocumento),'[A-Za-z\.\-\,\(\)\@\_\Ã\ ]+','') ELSE
		...
		REGEXP_REPLACE(CONCAT(' ', UPPER(goetu.ndocumento)), '[A-Za-z\.\-\,\(\)\@\_\Ã\ ]+', '') AS documento_broker,
...
SELECT 'SIENFO' base_origen,
	    CAST(a.id_alumno AS VARCHAR) codigo_origen,
		 CONCAT((CASE WHEN (d.nombre = 'D.N.I.') THEN 'DNI'
				...
				REGEXP_REPLACE(CONCAT(' ', UPPER(a.nrodoc)), '[A-Za-z\.\-\,\(\)\@\+\/\Ã\ ]+', ''),
		...
		CONCAT(RPAD(CASE WHEN (d.nombre = 'D.N.I.') THEN 'DNI'
				...
				LPAD((CASE WHEN d.nombre IN ('D.N.I.', 'L.C.', 'L.E.', 'C.I.', 'CUIT', 'CUIL') THEN REGEXP_REPLACE(UPPER(a.nrodoc),'[A-Za-z\.\-\,\(\)\@\+\/\Ã\ ]+','') ELSE
...
SELECT 'CRMEMPLEO' base_origen,
	...
	CONCAT(
		...
		REGEXP_REPLACE(UPPER(CAST(CASE WHEN REGEXP_LIKE(cea.numero_de_documento__c, '@') THEN
					...
					ELSE split_part(cea.numero_de_documento__c, '..', 1) END AS VARCHAR)),'[A-Za-z\.\-\,\(\)\@\_\Ã\ ]+', ''),
		(CASE
	...
	REGEXP_REPLACE(UPPER(CAST(CASE WHEN REGEXP_LIKE(cea.numero_de_documento__c, '@') THEN
					CASE WHEN REGEXP_LIKE(LOWER(cea.numero_de_documento__c), '\.ar') THEN
						split_part(split_part(cea.numero_de_documento__c, '@', 2),'.',4)
					ELSE split_part(split_part(cea.numero_de_documento__c, '@', 2),'.',3) END
					ELSE split_part(cea.numero_de_documento__c, '..', 1) END AS VARCHAR)),'[A-Za-z\.\-\,\(\)\@\_\Ã\ ]+', '') AS documento_broker,
...
WITH
  iel AS (
   ...
   , REGEXP_REPLACE(CONCAT(' ', CAST(nrodocumento AS VARCHAR)), '[A-Za-z\.\-\,\(\)\@\_\Ã±\Ã\ ]+', '') AS nrodocumento
   , nivel
   , sede
   , curso
...
SELECT tmp.base_origen_ok base_origen,
		...
		REGEXP_REPLACE(CONCAT(' ', CAST(tmp.nrodocumento AS VARCHAR)), '[A-Za-z\.\-\,\(\)\@\_\Ã±\ ]+', '') documento_original
FROM tmp
...
(SELECT replace(replace(vec.broker_id_din, ' ', '') || '-' || replace(vec.base_origen, ' ', ''), ' ', '') AS id_vecino,
	   ...
	   vec.apellido,
	   vec.fecha_nacimiento,
	   CASE WHEN vec.genero_broker IN ('F', 'M', 'X') THEN vec.genero_broker
        WHEN vec.genero_broker LIKE 'Masculino' THEN 'M'
        WHEN vec.genero_broker LIKE 'Femenino' THEN 'F'
        ELSE 'X' END AS genero_broker,
	   vec.nacionalidad,
	   vec.descrip_nacionalidad,
	   vec.nacionalidad_broker,
	   vec.nombre_valido,
...
SELECT tvc.id_vecino,
	   ...
	   tvc.renaper_valido
FROM tmp_vec_renaper tvc
WHERE documento_broker IS NOT NULL AND LENGTH(TRIM(documento_broker))>0
--</sql>--
...
SELECT
	 ...
	 REGEXP_REPLACE(CONCAT(' ', CAST(gu.ndocumento AS VARCHAR)), '[A-Za-z\.\-\,\(\)\@\_\Ã±\ ]+', '') documento_broker,
...
SELECT
	...
	REGEXP_REPLACE(CONCAT(' ', CAST(usuario.username AS VARCHAR)), '[A-Za-z\.\-\,\(\)\@\_\Ã±\ ]+', '') documento_broker,
...
SELECT
	...
	REGEXP_REPLACE(CONCAT(' ', CAST(nmpd.nro_documento AS VARCHAR)), '[A-Za-z\.\-\,\(\)\@\_\Ã±\ ]+', '') documento_broker,
...
CREATE TABLE "caba-piba-staging-zone-db"."tbp_typ_def_puestos" AS
 SELECT ciuo_88_code AS id_puesto,
...
WHERE codigo='3139'
 ...
 SELECT m.puesto_id AS id_puesto,
 	m.ciuo_08_code,
 	m.ciuo_08_titulo_sp,
...
--<sql>--
DROP TABLE IF EXISTS `caba-piba-staging-zone-db`.`tbp_typ_tmp_oportunidad_laboral_conocimientos`;
--</sql>--
...
SELECT
	...
	CAST(jo.id AS VARCHAR) oportunidad_laboral_id,
...
SELECT
	'ASI' base_origen,
	CAST(olv.id AS VARCHAR) AS oportunidad_laboral_id,
	-- idioma requerido para el puesto
	olv.idioma_requerido
	FROM (SELECT id,
		'ESPAÃOL' idioma_requerido
		FROM "caba-piba-raw-zone-db"."ofertas_laborales_view" WHERE UPPER(idiomas_nombre) LIKE '%ESPAÃOL%'

		UNION

		SELECT id,
		'INGLÃS' idioma_requerido
		FROM "caba-piba-raw-zone-db"."ofertas_laborales_view" WHERE UPPER(idiomas_nombre) LIKE '%INGLÃS%'

		UNION

		SELECT id,
		'PORTUGUÃS' idioma_requerido
		FROM "caba-piba-raw-zone-db"."ofertas_laborales_view" WHERE UPPER(idiomas_nombre) LIKE '%PORTUGUÃS%'

		UNION

		SELECT id,
		'ALEMÃN' idioma_requerido
		FROM "caba-piba-raw-zone-db"."ofertas_laborales_view" WHERE UPPER(idiomas_nombre) LIKE '%ALEMÃN%'

		UNION

		SELECT id,
		'FRANCÃS' idioma_requerido
		FROM "caba-piba-raw-zone-db"."ofertas_laborales_view" WHERE UPPER(idiomas_nombre) LIKE '%FRANCÃS%'

		UNION

		SELECT id,
		'ITALIANO' idioma_requerido
		FROM "caba-piba-raw-zone-db"."ofertas_laborales_view" WHERE UPPER(idiomas_nombre) LIKE '%ITALIANO%'
...
--<sql>--
CREATE TABLE "caba-piba-staging-zone-db"."tbp_typ_tmp_oportunidad_laboral_conocimientos" AS
WITH oportunidad_laboral_conocimientos AS (
SELECT
	'PORTALEMPLEO' base_origen,
	CAST(inf_a.informatic_id AS VARCHAR)||'-'||CAST(kas.id AS VARCHAR) AS informatica_conocimiento_id,
	CAST(jo.id AS VARCHAR) AS oportunidad_laboral_id,
	inf_a.informatic_id AS informatica_id,
	kas.id AS conocimiento_id,
	'SOFTWARE CONTABLE' AS tipo_conocimiento,
	UPPER(kas.name) AS descripcion_conocimiento
FROM "caba-piba-raw-zone-db"."portal_empleo_job_offers" jo
JOIN "caba-piba-raw-zone-db"."portal_empleo_job_postings" jp ON (jo.id=jp.job_offer_id)
JOIN "caba-piba-raw-zone-db"."portal_empleo_informatics" inf ON (jp.informatic_id=inf.id)
JOIN "caba-piba-raw-zone-db"."portal_empleo_informatics_accouting" inf_a ON (inf_a.informatic_id = jp.informatic_id)
JOIN "caba-piba-raw-zone-db"."portal_empleo_know_account_systems" kas ON (inf_a.accounting_knowledge_id = kas.id)

UNION

SELECT
	'PORTALEMPLEO' base_origen,
	CAST(inf_db.informatic_id AS VARCHAR)||'-'||CAST(dbk.id AS VARCHAR) AS informatica_conocimiento_id,
	CAST(jo.id AS VARCHAR) AS oportunidad_laboral_id,
	inf_db.informatic_id AS informatica_id,
	dbk.id AS conocimiento_id,
	'BASES DE DATOS' AS tipo_conocimiento,
	UPPER(dbk.name) AS descripcion_conocimiento
FROM "caba-piba-raw-zone-db"."portal_empleo_job_offers" jo
JOIN "caba-piba-raw-zone-db"."portal_empleo_job_postings" jp ON (jo.id=jp.job_offer_id)
JOIN "caba-piba-raw-zone-db"."portal_empleo_informatics" inf ON (jp.informatic_id=inf.id)
JOIN "caba-piba-raw-zone-db"."portal_empleo_informatics_databases" inf_db ON (inf_db.informatic_id = jp.informatic_id)
JOIN "caba-piba-raw-zone-db"."portal_empleo_database_knowledge" dbk ON (inf_db.database_id = dbk.id)

UNION

SELECT
	'PORTALEMPLEO' base_origen,
	CAST(inf_off.informatic_id AS VARCHAR)||'-'||CAST(off_sof.id AS VARCHAR) AS informatica_conocimiento_id,
	CAST(jo.id AS VARCHAR) AS oportunidad_laboral_id,
	inf_off.informatic_id AS informatica_id,
	off_sof.id AS conocimiento_id,
	'SOFTWARE OFIMATICA' AS tipo_conocimiento,
	UPPER(off_sof.name) AS descripcion_conocimiento
FROM "caba-piba-raw-zone-db"."portal_empleo_job_offers" jo
JOIN "caba-piba-raw-zone-db"."portal_empleo_job_postings" jp ON (jo.id=jp.job_offer_id)
JOIN "caba-piba-raw-zone-db"."portal_empleo_informatics" inf ON (jp.informatic_id=inf.id)
JOIN "caba-piba-raw-zone-db"."portal_empleo_informatics_office" inf_off ON (inf_off.informatic_id = jp.informatic_id)
JOIN "caba-piba-raw-zone-db"."portal_empleo_office_software" off_sof ON (inf_off.office_id = off_sof.id)

UNION

SELECT
	'PORTALEMPLEO' base_origen,
	CAST(inf_pro.informatic_id AS VARCHAR)||'-'||CAST(pro_k.id AS VARCHAR) AS informatica_conocimiento_id,
	CAST(jo.id AS VARCHAR) AS oportunidad_laboral_id,
	inf_pro.informatic_id AS informatica_id,
	pro_k.id AS conocimiento_id,
	'PROGRAMACION' AS tipo_conocimiento,
	UPPER(pro_k.name) AS descripcion_conocimiento
FROM "caba-piba-raw-zone-db"."portal_empleo_job_offers" jo
JOIN "caba-piba-raw-zone-db"."portal_empleo_job_postings" jp ON (jo.id=jp.job_offer_id)
JOIN "caba-piba-raw-zone-db"."portal_empleo_informatics" inf ON (jp.informatic_id=inf.id)
JOIN "caba-piba-raw-zone-db"."portal_empleo_informatics_programming" inf_pro ON (inf_pro.informatic_id = jp.informatic_id)
JOIN "caba-piba-raw-zone-db"."portal_empleo_programming_knowledge" pro_k ON (inf_pro.programming_knowledge_id = pro_k.id)
)
SELECT *
...
inf.informatics AS informatica,
inf.internet AS internet,
inf.specific_knowledge AS conocimiento_especifico_requerido
...
UPPER(TRIM(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(COALESCE(et3.organizacion_empleadora, 'OTRA EXPERIENCIA'), '[\\\\|_-]', ' '), ',|/', ' y '), '[^a-zA-Z0-9Ã¡Ã©Ã­Ã³ÃºÃÃÃÃÃÃ±Ã ]', ''), ' +', ' '), '\+', ' '), '1/2', 'medio '), '100\%', ''), '^ ', ''), ' $'))) organizacion_empleadora_limpia,
...
CASE
	...
	OR et4.organizacion_empleadora_limpia IN ('OTRAS EXPERIENCIAS','OTRAS TAREAS','OTRAS ÃREAS','OTRO','OTROS','OTROS OFICINISTAS','OTROS TRABAJOS ANTERIORES') THEN 'OTRA EXPERIENCIA'
...
CASE
    WHEN ecr1.base_origen LIKE 'PORTALEMPLEO' OR ecr1.base_origen LIKE 'ASI' THEN 'PORTAL DE EMPLEO'
...
WITH aa_ab AS (
	...
	FROM "caba-piba-staging-zone-db"."afip_agip_altas_bajas" ab
...
(
	...
	SELECT
		...
		rf_sin_oportunidad.fecha_de_movimiento_origen,
...
WHERE cuit_del_empleador IS NOT NULL
GROUP BY cuit_del_empleador
UNION
SELECT
...
-- POSTULACIONES TMP
--<sql>--
DROP TABLE IF EXISTS `caba-piba-staging-zone-db`.`tbp_typ_tmp_postulaciones`;
--</sql>--

--<sql>--
CREATE TABLE "caba-piba-staging-zone-db"."tbp_typ_tmp_postulaciones" AS
-- PORTALEMPLEO
SELECT vec.base_origen,
	...
	-- 1 si fue tuvo entrevista, 0 en caso contrario
	ja.contacted AS "tuvo_entrevista",
	CASE
	WHEN ja.contacted = 1 OR ja.hired = 1 THEN 'COMPLETA'
	ELSE 'PENDIENTE'
	END estado_entrevista,
	CAST(jp.job_offer_id AS VARCHAR) oportunidad_laboral_id_old
FROM "caba-piba-raw-zone-db"."portal_empleo_job_applications" ja
	...
	LEFT JOIN "caba-piba-raw-zone-db"."portal_empleo_job_application_comments" jac ON (ja.id = jac.job_application_id)
	...
	ELSE 'PENDIENTE'
	END,
	CAST(jp.job_offer_id AS VARCHAR)
UNION
-- CRMEMPLEO
SELECT vec.base_origen,
	 vec.id_vecino,
	...
	CAST(COALESCE(p.createddate,e.createddate) AS DATE) AS "fecha_notificacion",
	-- 1 si fue contratado, 0 en caso contrario
	CASE WHEN p.estado__c LIKE 'Contratado' THEN 1 ELSE 0 END  AS "consiguio_trabajo",
	-- 1 si fue tuvo entrevista, 0 en caso contrario
	CASE WHEN p.estado__c IN ('Contratado', 'Entrevista realizada', 'Enviado al solicitante', 'Contactado')
		THEN 1 ELSE 0 END AS "tuvo_entrevista",

	CASE
	WHEN p.estado__c IN ('Contratado', 'Entrevista realizada', 'Enviado al solicitante', 'Contactado') THEN 'COMPLETA'
	ELSE 'PENDIENTE'
	END estado_entrevista,
	CAST(anu.id AS VARCHAR) oportunidad_laboral_id_old
...
SELECT vec.base_origen,
	...
	ELSE 'PENDIENTE'
	END estado_entrevista,
	(CAST(anu.id AS VARCHAR) || 'H') oportunidad_laboral_id_old
...
SELECT vec.base_origen,
	...
	c.consiguio_trabajo,
	-- 1 si fue tuvo entrevista, 0 en caso contrario
	CASE WHEN c.estado_entrevista IN ('INCOMPLETA', 'COMPLETA')
		THEN 1 ELSE 0 END AS "tuvo_entrevista",
	c.estado_entrevista,
	NULL AS  oportunidad_laboral_id_old
FROM (
...
GROUP BY
	...
	c.tipo_entrevista,
	c.consiguio_trabajo,
	CASE WHEN c.estado_entrevista IN ('INCOMPLETA', 'COMPLETA') THEN 1 ELSE 0 END,
	c.estado_entrevista
--</sql>--
...
FROM "caba-piba-staging-zone-db"."tbp_typ_tmp_postulaciones" p
	LEFT JOIN "caba-piba-staging-zone-db"."tbp_typ_def_oportunidad_laboral" ol ON (
		p.oportunidad_laboral_id_old = ol.id_old
...
SELECT
	...
	CASE
		...
		WHEN UPPER(empresa_limpia) LIKE '%100 BANCO%' OR UPPER(empresa_limpia) LIKE '%100 BANCO BANCO UNIVERSAL%' OR UPPER(empresa_limpia) LIKE '%100 BANCO NACIONAL%' OR UPPER(empresa_limpia) LIKE '%100 BANCO UNIVESAL Y SA%' OR UPPER(empresa_limpia) LIKE '%100 BANCO Y BANCO UNIVERSAL%' OR UPPER(empresa_limpia) LIKE '%100 BANCO Y BANCO UNIVERSAL CA%' OR UPPER(empresa_limpia) LIKE '%100 BANCO Y BANCO UNIVERSAL Y CA%' OR UPPER(empresa_limpia) LIKE '%100BANCO Y BANCO UNIVERSAL Y VENEZUELA%' OR UPPER(empresa_limpia) LIKE '%100X100 BANCO Y BANCO UNIVERSAL%' THEN '100%BANCO, BANCO UNIVERSAL, C.A.'
	    WHEN UPPER(empresa_limpia) LIKE '%12 2017 Y 03 2018 VIRI BURGER BERNAL%' THEN 'VIRI BURGER S.R.L.'
	    WHEN UPPER(empresa_limpia) LIKE '%123 SEGURO%' OR UPPER(empresa_limpia) LIKE '%123 SEGURO EUROSECURITY%' OR UPPER(empresa_limpia) LIKE '%123SEGURO%' OR UPPER(empresa_limpia) LIKE '%123SEGURO EURO SECURITY SA%' THEN '123SEGUROS'
	    WHEN UPPER(empresa_limpia) LIKE '%1RA ESCUELA PRIV DE PSICO SOCIAL P RIVIERE%' THEN 'ESCUELA DE PSICOLOGÃA SOCIAL PICHON RIVIERE'
	    WHEN UPPER(empresa_limpia) LIKE '%1SOLUCIONES MEDICAS HOSPITALARIAS JA Y CA%' THEN 'SOLUCIONES MÃDICAS HOSPITALARIAS JA, C.A.'
	    WHEN UPPER(empresa_limpia) LIKE '%2 CERO VALMORAL CONSULTORES SA%' THEN 'VALMORAL CONSULTORES S.A.'
	    WHEN UPPER(empresa_limpia) LIKE '%2 CONGRESO IBEROAMERICANO DE DBT Y CABA%' THEN ''
	    WHEN UPPER(empresa_limpia) LIKE '%2017 ZANELLA HNOS Y CIA%' OR UPPER(empresa_limpia) LIKE '%ZANELLA%' OR UPPER(empresa_limpia) LIKE '%ZANELLA HMN%' OR UPPER(empresa_limpia) LIKE '%ZANELLA HNOS%' OR UPPER(empresa_limpia) LIKE '%ZANELLA HNOS SA%' OR UPPER(empresa_limpia) LIKE '%ZANELLA HNOS Y CIA%' OR UPPER(empresa_limpia) LIKE '%ZANELLA HNOS Y CIA SA%' THEN 'ZANELLA HNOS & CIA SACIFI'
...
WITH c1 AS (
  SELECT
    org.codigo_de_actividad AS id_actividad,
    org.descripcion_actividad,
    m.codigo_clae,
    m.id_sector_productivo
...
c3 AS (
  SELECT
    ...
    END AS codigo_clae_clean,
    c1.codigo_clae,
    c1.id_sector_productivo
  FROM c1
  LEFT JOIN "caba-piba-raw-zone-db"."tbp_typ_tmp_nomenclador_actividades_economicas" ae ON (c1.id_actividad = TRY_CAST(ae.cod_actividad_afip_o_naes AS INT) OR c1.id_actividad = TRY_CAST(ae.cod_actividad_agip_o_naecba AS INT))
),
c4 AS (
  SELECT
    CASE
      WHEN c3.codigo_clae_clean IS NULL THEN c3.id_actividad
      ELSE c3.codigo_clae_clean
    END AS id_actividad,
    c3.descripcion_actividad,
    m1.id_sector_productivo
  FROM c3
...
c5 AS (
  SELECT
    ...
    CASE
      WHEN c4.id_sector_productivo IS NULL THEN m1.id_sector_productivo
      ELSE c4.id_sector_productivo
    END AS id_sector_productivo,
    m1.sector_productivo
  FROM c4
  LEFT JOIN "caba-piba-staging-zone-db"."tbp_typ_def_match_sector_estrategico_sector_productivo" m1 ON (SUBSTRING(CAST(c4.id_actividad AS VARCHAR), 1, 4) = REPLACE(m1.codigo_4_digitos_clanae, '.', ''))
)
SELECT
  c5.id_actividad,
  c5.descripcion_actividad,
  c5.id_sector_productivo
FROM c5
WHERE c5.id_sector_productivo IS NOT NULL
GROUP BY 1, 2, 3
--</sql>--
...
--<sql>--
CREATE TABLE "caba-piba-staging-zone-db"."tbp_typ_tmp_areas_de_interes" AS WITH areas_de_interes AS (
	...
	SELECT CAST(ia.id AS VARCHAR) id,
		'PORTALEMPLEO' base_origen,
		CAST(ia.name AS VARCHAR) descripcion
	FROM "caba-piba-raw-zone-db"."portal_empleo_mtr_interest_areas" ia
)
SELECT *
...
--<sql>--
CREATE TABLE "caba-piba-staging-zone-db"."tbp_typ_tmp_oportunidad_laboral_areas_de_interes" AS WITH oportunidad_laboral_areas_de_interes AS (
	...
	-- AREAS DE INTERES DE LA OFERTAS LABORALES, POR EJEMPLO: 'InformÃ¡tica / IT / Sistemas', 'AtenciÃ³n al Cliente', 'Gastronomia'
	SELECT CAST(jo.id AS VARCHAR) id_oportunidad_laboral,
		'PORTALEMPLEO' base_origen,
		CAST(ia.id AS VARCHAR) id_areas_de_interes
	FROM "caba-piba-raw-zone-db"."portal_empleo_mtr_interest_areas" ia
		JOIN "caba-piba-raw-zone-db"."portal_empleo_job_offers" jo ON (ia.id = jo.area_id)
)
SELECT *
...
--<sql>--
CREATE TABLE "caba-piba-staging-zone-db"."tbp_typ_tmp_curriculum_areas_de_interes" AS WITH curriculum_areas_de_interes AS (
	...
	SELECT CAST(cv.id AS VARCHAR) id_curriculum,
		CAST(ia.id AS VARCHAR) id_areas_de_interes,
		'PORTALEMPLEO' base_origen
	FROM "caba-piba-raw-zone-db"."portal_empleo_mtr_interest_areas" ia
...
--<sql>--DROP TABLE IF EXISTS `caba-piba-staging-zone-db`.`tbp_typ_def_vecino_nivel_educativo`;--</sql>--
--<sql>--
CREATE TABLE "caba-piba-staging-zone-db"."tbp_typ_def_vecino_nivel_educativo" AS
